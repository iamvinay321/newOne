...
*/
      ...
      if ('V_Key_Names' in this.PVP) {
        ...
        for (let i = 0; i < this.V_KEY_NAMEA.length; i++) {
      ...
      if ('0' in this.PVP) {
        ...
        for (let i = 0; i < this.V_KEY_VALUEA.length; i++) {
      ...
      const l = this.s_data.length - this.Field_Data.length;
      ...
      if (this.displayedColumnsClone.length > 0) {
        ...
        const index2 = this.displayedColumnsClone.indexOf('V_PRCS_ID');
        if (index2 !== -1) {
          this.displayedColumnsClone.splice(index2, 1);
          this.displayedColumns = this.displayedColumnsClone;
        }
        const index3 = this.displayedColumnsClone.indexOf('V_SRC_CD');
        if (index3 !== -1) {
          this.displayedColumnsClone.splice(index3, 1);
          this.displayedColumns = this.displayedColumnsClone;
        }
        const index4 = this.displayedColumnsClone.indexOf('V_SRVC_CD');
        if (index4 !== -1) {
          this.displayedColumnsClone.splice(index4, 1);
          this.displayedColumns = this.displayedColumnsClone;
        }
        const index5 = this.displayedColumnsClone.indexOf('V_Key_Value');
        if (index5 !== -1) {
          this.displayedColumnsClone.splice(index5, 1);
          this.displayedColumns = this.displayedColumnsClone;
        }
        const index6 = this.displayedColumnsClone.indexOf('V_Key_Name');
        if (index6 !== -1) {
    ...
    this.http.get(this.apiUrlGet + 'V_SRVC_CD=' + this.col4 + '&V_APP_CD=' + this.col2 + '&V_PRCS_CD=' + this.col3 + '&V_SRC_CD=' + this.col1 + '&REST_Service=Artifacts&Verb=GET').subscribe(
      res => {
       // this.oldFileData=res.js;
        ...
        console.log(this.artfct_nm);
        for (let i = 0; i < this.artfct_nm.length; i++) {
          this.filesdata = {};
  ...
  submitbtn_click() {
    ...
    for (let j = 0; j < this.displayedColumns.length - 1; j++) {
      ...
      for (let i = 0; i < this.td.length; i++) {
        const k = this.displayedColumns[j].split(' ').join('_');
        if (r === 1) {
          r = 2;
          if (q === 1) {
            this.tnames += '{"' + k + '":';
...
tableData: string[] = [];
    ...
    if (this.V_TABLE_NAME !== '') {
      ...
      this.http.get(ul).subscribe(
        res => {
          ...
          this.td = [];
          const l = this.initvalues1[this.displayedColumns[0]].length;
          console.log(l);
          for (let j = 0; j < l; j++) {
            this.tm = {};
  ...
  sendFieldValue(r: number) {
    if (this.td[r]['up'] === true) {
      ...
      this.Field_Names = '';
      this.Field_Values = '';
      for (let s = 0; s < this.displayedColumns.length - 1; s++) {
  ...
  deleteorCancelvalue(r: number) {
    ...
    for (let i = r; i < this.td.length; i++) {
      this.td[i]['id'] = i;
